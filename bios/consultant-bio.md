# Consultant Bio

- **Purpose:** the professional-services introduction for formal business documents.
- **Audience:** proposal introductions, capability statements, discovery-call briefs, vendor profiles, RFP submissions.
- **Length:** 100–180 words.
- **Canonical sources:** `docs/career-positioning.md` (audience framing), `docs/achievement-library.md` (Tier A/B evidence), `docs/certifications.md`, `experience/`.

## Approved Wording (third person)

> Tyrone Nelms is an Operations & Business Systems Architect and the founder of AJ Digital. He translates operational knowledge into requirements, process and information models, system specifications, implementation plans, and governed AI-enabled workflows.
>
> His work is grounded in 15+ years of operations — enterprise commercial-account support, healthcare provider services, sales and office administration, field service for ADA and mobility equipment, event production, and digital delivery. That range gives him first-hand knowledge of the workflows systems must support, from customer calls and CRM records to field logistics and approval-sensitive handoffs.
>
> He holds Google professional certificates in Project Management, UX Design, and Digital Marketing & E-commerce. His working standard is evidence discipline: recommendations documented, processes mapped before automation, and no claimed outcome without support.

## Optional Wording

For RFPs requiring a recognition line (verified record only):

> At Alorica, he was recognized as Employee of the Month, quarterly top performer, and subject-matter expert, maintaining over 99% adherence and two years of perfect attendance.

## Open Questions

- Add insurance/business-entity details if RFPs require them (business facts, not career facts — confirm with owner before adding).

## Maintenance Rules

- This bio anchors the capability statements in `capability-statements/`; keep the first paragraph aligned with their Overview sections.
- No engagement-model or pricing language here — that belongs to the capability statements.
