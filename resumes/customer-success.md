# Tyrone Nelms

**Customer Success Manager**

Miami / Hialeah, Florida  
Email: Tyrone.nelms87@gmail.com  
Phone: 786-280-4470

## Professional Summary

Customer experience and operations professional with enterprise support experience across commercial accounts, healthcare provider services, escalation handling, documentation, CRM systems, technical support, and customer resolution. Recognized for high performance, attendance, adherence, team support, and mentoring, with the ability to communicate clearly with customers, internal teams, and stakeholders.

## Core Competencies

- Customer Success & Account Support
- Escalation Handling
- First-Call Resolution
- CRM Systems (Salesforce, HubSpot, RightNow)
- Issue Research & Resolution
- Quality Assurance
- Documentation
- Training & Mentoring
- Relationship Building

## Professional Experience

### AJ Digital / Freelance Consulting

**Founder / Operations & Digital Marketing Specialist**  
April 2020 – Present

- Manage client relationships from discovery through delivery for founder-led and service-based businesses.
- Design customer experience improvements, response workflows, and CRM processes for clients.
- Support customer engagement through content, communication systems, and follow-up workflows.

### Florida Ramp & Lift

**Independent Contractor — Marketing & Operations Professional**  
July 2023 – Present

- Coordinate directly with customers on accessibility service delivery, scheduling, and issue resolution.
- Support service quality and communication in a field-service environment.

### Miami Spin 360

**Event Planner / Creative Operations**  
February 2022 – Present

- Deliver client-facing event services, managing expectations and on-site experience end to end.

### UnitedHealthcare

**Provider Services Representative**  
2017 – 2019 (approximate)

- Answered inbound calls to provide information, answer provider questions, and discuss payment options.
- Researched customer and provider issues to understand problems and develop successful resolutions.
- Delivered positive customer experiences by applying quality assurance practices.
- Handled difficult calls with professionalism and strong telephone etiquette.

### Alorica

**Commercial Account Specialist Sr. Rep**  
2015 – 2017 (approximate)

- Resolved commercial account issues using Salesforce, RightNow, SharePoint, and internal systems (STMS, OMS, RIO, DocDNA).
- Made outbound calls to clients, authorized dealers, and technicians to resolve caller questions and issues.
- Supported first-call resolution and documented contracts, caller issues, required steps, and final resolutions.
- Assisted with supervisor callbacks, escalations, and special cases requiring immediate attention.
- Recognized as Employee of the Month (March 2015), quarterly top performer, TPSM Team SME, and team lead.
- Achieved perfect attendance for two years and maintained over 99% adherence every month.
- Trained, mentored, and coached team members; provided floor support for new hires.

### AHLO Inc.

**Consolidated Operations / Sales / Office Administration**  
Multiple periods between 2006 and 2022 (consolidated entry)

- Placed orders and answered customer questions in person, by email, and over the phone.
- Contacted new and existing customers to communicate product benefits.
- Monitored customer order processes and addressed customer issues.

### TigerDirect.com

**Customer Service Representative**  
February 2008 – April 2011

- Handled inbound customer service calls and managed customer accounts.
- Provided low-level technical support and supported inbound sales.

### Earlier Experience

Medical records and data entry at Doctors Diabetics (2006).

## Certifications

- Google Project Management Professional Certificate — Coursera
- Google UX Design Professional Certificate — Coursera
- Google Digital Marketing & E-commerce Professional Certificate — Coursera

## Education

American Academy — High School Diploma, 2005

## Technology & Tools

Salesforce, HubSpot, RightNow, SharePoint, STMS, OMS, RIO, DocDNA, Microsoft 365, Google Workspace, ClickUp, Asana, Notion.
