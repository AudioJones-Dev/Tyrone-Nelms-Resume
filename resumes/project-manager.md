# Tyrone Nelms

**Project Manager**

Miami / Hialeah, Florida  
Email: Tyrone.nelms87@gmail.com  
Phone: 786-280-4470

## Professional Summary

Project management and operations professional with experience planning and delivering client projects across digital marketing, business operations, creative production, and field service. Skilled in stakeholder communication, cross-functional coordination, task and timeline management, documentation, and status reporting. Google-certified in project management, with hands-on experience running client work in ClickUp, Asana, and Notion.

## Core Competencies

- Project Planning & Execution
- Stakeholder Communication
- Cross-Functional Coordination
- Task & Timeline Management
- Process Mapping & Documentation
- Status Reporting
- Client Discovery & Implementation Support
- ClickUp, Asana, Notion

## Professional Experience

### AJ Digital / Freelance Consulting

**Founder / Operations & Digital Marketing Specialist**  
April 2020 – Present

- Manage client projects end to end across ClickUp, Asana, Notion, Google Workspace, and Microsoft 365.
- Run client discovery, translate business problems into implementation plans, and support delivery.
- Streamline workflows and improve team efficiency through documentation and process design.
- Coordinate content creation, advertising, and campaign execution across contributors and deadlines.

### Florida Ramp & Lift

**Independent Contractor — Marketing & Operations Professional**  
July 2023 – Present

- Support logistics and coordination for field work, including scheduling and customer communication.
- Contribute to operational process improvements in a service-delivery environment.

### Miami Spin 360

**Event Planner / Creative Operations**  
February 2022 – Present

- Plan and execute client events with fixed dates, coordinating vendors, equipment, and on-site delivery.
- Manage client-facing production needs under real-time event constraints.

### AHLO Inc.

**Consolidated Operations / Sales / Office Administration**  
Multiple periods between 2006 and 2022 (consolidated entry)

- Monitored customer order processes from placement through resolution.
- Coordinated across sales, warehouse, and administrative functions.
- Directed daily operations and supported team alignment with business goals.

### Alorica

**Commercial Account Specialist Sr. Rep**  
2015 – 2017 (approximate)

- Documented caller questions, contracts, required steps, and final resolutions with high accuracy.
- Coordinated escalations and supervisor callbacks across teams; served as TPSM Team SME.
- Trained and mentored team members and hosted team huddles to communicate updates.

### UnitedHealthcare

**Provider Services Representative**  
2017 – 2019 (approximate)

- Researched provider issues, coordinated resolutions, and communicated outcomes professionally.

### Earlier Experience

Customer service and records roles at TigerDirect.com (2008 – 2011) and Doctors Diabetics (2006).

## Certifications

- Google Project Management Professional Certificate — Coursera
- Google UX Design Professional Certificate — Coursera
- Google Digital Marketing & E-commerce Professional Certificate — Coursera

## Education

American Academy — High School Diploma, 2005

## Technology & Tools

ClickUp, Asana, Notion, Google Workspace, Microsoft 365, Salesforce, HubSpot, SharePoint, Zapier, Canva, Figma.
